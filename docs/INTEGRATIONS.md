...
- Optional custom properties (create once in HubSpot; writes degrade gracefully
  if they don't exist): `anaga_disposition` (text), `anaga_intent_score` (number),
  `anaga_last_call` (datetime), `anaga_dnd` (checkbox — rename with `CRM_DND_PROPERTY`).
...
// timing-safe compare against req.headers['x-anaga-signature-256']
```
...
1. verify `X-Anaga-Signature-256` against `DIAL_QUEUE_SECRET`;
...
curl -s -X POST $BASE/api/calls/outcome \
  ...
  -d '{"call":{"id":"smoke-1","disposition":"booked"},
       ...
       "history":[{"role":"agent","text":"Hi, I am Anaga, an AI assistant from Modcon Builders. Do you have a minute?"},
...
Its design comes from `design-system/anaga-console/`, generated by the
...
python3 .claude/skills/ui-ux-pro-max/scripts/search.py \
  ...
  --design-system --variance 3 --motion 2 --density 8 -p "Anaga Console" --output-dir "$(pwd)"
```
...
| Console design system | `design-system/anaga-console/` |
