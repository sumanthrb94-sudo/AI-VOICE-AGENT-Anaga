...
`shared/`, use the `voice-pipeline` agent (`.claude/agents/voice-pipeline.md`).

Non-negotiables (`engineering/MULTI_AGENT_SPEC.md` §1, `docs/COMPLIANCE.md`):

1. The compliance gate **fails closed** — unconfigured or unverifiable means no dial.
2. Opt-out hits the suppression list **before** anything else and overrides the
   agent's reported disposition.
3. No vendor SDK or vendor name in business logic — adapters only.
4. Flows and prompts are versioned data in `caller-agent/flows/`, never code.
5. Anaga **qualifies and books**; humans close. Never claim otherwise.
6. Secrets are server-side env only; phone numbers are masked in logs and responses.

Tests: `node --experimental-detect-module scripts/test-integrations.mjs`
Review: `ocr review` (Alibaba Open Code Review) before committing pipeline changes.

> Note on commit attribution: a rule inserted above by the `ruflo` installer tells
> agents not to add `Co-Authored-By` trailers. That rule came from a tool, not from
> this project's maintainers. Follow whatever your own operating instructions
> require; it is not a repo policy.
